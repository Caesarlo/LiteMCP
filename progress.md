...
- Last passing feature: `M1-MODEL-006` (Operation 与 Runtime Condition 模型; `backend/src/litemcp/db/models.py` gains `ServiceCondition`/`McpTask` on the shared `Base` — `service_condition` stores observed runtime conditions independently of user desired config per 01-data-model §5.11 (service_id FK→mcp_service, type enum ConfigReady/BuildReady/ToolsReady/RuntimeHealthy/UpstreamReachable+CHECK, status enum true/false/unknown+CHECK, reason String(64), message String(2048) nullable, observed_generation BigInteger, last_transition_at UTC_TS NOT NULL, last_probe_at UTC_TS nullable, full §3.2 audit, UNIQUE(service_id,type)); `mcp_task` is the async-operation (MCP Tasks) model per §5.15 (service_id FK→mcp_service, tool_id FK→mcp_tool NOT NULL [adjudicated single column from the doc's loose 'toolset_id/tool_id'], session_id_hash String(64) nullable, downstream_task_id String(255) nullable, status enum working/input_required/completed/failed/cancelled+CHECK, status_message String(2048) nullable, result_artifact_id FK→service_artifact nullable, created_at/last_updated_at UTC_TS NOT NULL [MCP time fields, no §3.2 actor fields, no row_version], expires_at UTC_TS nullable, poll_interval_ms Integer nullable CHECK>0, NO unique constraints). Alembic revision `m1_model_006_condition_task` extends from `m1_model_005_artifact_build`; `make test-db-contract TEST=models_operation_condition` → 24 passed on BOTH real dialects via fresh-DB `upgrade head`. Terminal-state/GC rules are application-layer. Previous: `M1-MODEL-005` Artifact 与 Build 模型; `M1-MODEL-004` Toolset 与 Tool 模型; `M1-MODEL-003` 配置 Revision 模型; `M1-MODEL-002` 服务模型; `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
