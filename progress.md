...
- Current blocker: None. Docker Desktop is running; compose `database`/`mysql`/`redis` are healthy (host ports 5433/3307/6379). M1 milestone complete (17/17 passing); M2 in progress: 6/18 passing.
- Last passing feature: `M2-AUTH-006` (实现 Access JWT 校验; `litemcp.auth.access_jwt.verify_access_jwt` checks signature/iss/aud/kid/typ/token_type/exp and loads current user status/role from the DB — application-layer only, no HTTP `get_current_user`).
- Next best step: activate `M2-AUTH-007` (实现 Refresh Token 轮换, priority 207, dependency `M2-AUTH-004` passing). Isolated TDD: test-writer for `backend/tests/auth/test_refresh_rotation.py`, controller RED, fresh implementer, controller GREEN, then the declared gate plus auth/core regressions.
