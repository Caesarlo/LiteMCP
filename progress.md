...
- Last passing feature: `M1-MODEL-005` (Artifact 与 Build 模型; `backend/src/litemcp/db/models.py` gains `ServiceArtifact`/`BuildRun` on the shared `Base` — `service_artifact` is the immutable object-record home per 01-data-model §5.5 (service_id FK→mcp_service, config_revision_id FK→`service_config_revision` nullable, kind/storage_backend/state ENUM_CODE+CHECK, object_key String(1024) [MySQL latin1_bin variant in the migration for the 3072-byte index limit], sha256 String(64), size_bytes BigInteger CHECK>=0, media_type String(128), format String(32), scan_report JSON_DOC nullable, retain_until UTC_TS nullable, full §3.2 audit, UNIQUE(storage_backend,object_key)); `build_run` is the build-attempt ledger per §5.6 (service_id FK→mcp_service, config_revision_id FK→`service_config_revision` NOT NULL, source_artifact_id FK→service_artifact NOT NULL, strategy ENUM_CODE fastmcp/descriptor/custom_adapter+CHECK, parser_version String(64), base_image_digest String(255), dependency_digest String(64) nullable, status ENUM_CODE queued/building/validating/succeeded/failed/cancelled/superseded+CHECK, output_artifact_id/log_artifact_id FK→service_artifact nullable, discovered_descriptor JSON_DOC nullable, error_code String(64) nullable, error_summary String(2048) nullable, started_at/finished_at UTC_TS nullable, full §3.2 audit, non-unique INDEX (service_id,status,created_at)). Alembic revision `m1_model_005_artifact_build` extends from `m1_model_004_toolset`; `make test-db-contract TEST=models_artifact_build` → 32 passed on BOTH real dialects via fresh-DB `upgrade head`. GC-safety is application-layer (no trigger). Previous: `M1-MODEL-004` Toolset 与 Tool 模型; `M1-MODEL-003` 配置 Revision 模型; `M1-MODEL-002` 服务模型; `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
