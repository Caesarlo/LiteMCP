...
- Current blocker: No code blocker. Controller-side rerun of Docker-backed verification is currently blocked by Docker Desktop named-pipe `Access is denied`; the implementer subagent reported the PostgreSQL/MySQL gate passing.
- Last passing feature: `M1-MODEL-007` (权限与 API Key 元数据模型; `backend/src/litemcp/db/models.py` gains `McpServicePermission`/`ApiKey` on the shared `Base` — `mcp_service_permission` is the explicit object-level permission ledger per 01-data-model §5.12 (service_id FK→mcp_service, principal_type enum user/team/everyone+CHECK, user_id FK→user nullable, team_id FK→team nullable, role enum editor/viewer+CHECK, principal_key String(80), full §3.2 audit, UNIQUE(service_id,principal_key); cross-field CHECKs: user_id consistency (principal_type='user' ⇔ user_id NOT NULL), team_id consistency, role scope (principal_type='user' OR role='viewer')); `api_key` is the non-secret API-key metadata per §5.13 (service_id FK→mcp_service, public_id String(32) UNIQUE, display_prefix String(32), secret_hash String(64) UNIQUE, hash_algorithm String(32) [plain, upgradable], pepper_version String(64) nullable, name String(128), status enum active/revoked+CHECK, expires_at UTC_TS nullable CHECK > created_at, last_used_at UTC_TS nullable, last_used_ip_hash String(64) nullable, revoked_at UTC_TS nullable CHECK (status<>'revoked' OR revoked_at NOT NULL), revoked_by FK→user nullable, rate_limit_qps Numeric nullable CHECK >0, rate_limit_burst Integer nullable CHECK >=1, full §3.2 audit). Alembic revision `m1_model_007_permission_key` extends from `m1_model_006_condition_task`; `make test-db-contract TEST=models_permission_key` → 34 passed on BOTH real dialects via fresh-DB `upgrade head`. Controller-approved supporting change: four non-unique query indexes (permission visibility/writability lookups + api_key status/expiry scan). API key plaintext is never persisted (application-layer guarantee). Previous: `M1-MODEL-006` Operation 与 Runtime Condition 模型; `M1-MODEL-005` Artifact 与 Build 模型; `M1-MODEL-004` Toolset 与 Tool 模型; `M1-MODEL-003` 配置 Revision 模型; `M1-MODEL-002` 服务模型; `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
