...
- Last passing feature: `M1-MODEL-008` (审计事件与 Outbox 模型; `backend/src/litemcp/db/models.py` gains `AuditEvent`/`Outbox` on the shared `Base` — `audit_event` is the append-only business-evidence ledger per 01-data-model §5.14 (id, occurred_at UTC_TS, request_id String(128), actor_type enum user/api_key/system/anonymous+CHECK, actor_id String(128) nullable, action String(64), resource_type String(32), resource_id String(128) nullable, service_id ID nullable NO FK, result enum success/denied/failed+CHECK, reason_code/source_ip String(64) nullable, user_agent String(1024) nullable, changes/metadata JSON_DOC nullable, previous_event_hash/event_hash String(64) nullable [optional anti-tamper chain]; NO §3.2 audit columns — the table IS the audit record; indexes occurred_at / (service_id,occurred_at) / (actor_type,actor_id,occurred_at)); `outbox` is the transactional delivery queue, controller-adjudicated structure from ADR-0003 + 03-service-crud §6.3 + 07-observability L171-L172 (id, event_type String(64), service_id ID nullable NO FK, requested_generation BigInteger nullable, operation_kind String(64) nullable, payload JSON_DOC nullable, status enum pending/in_flight/done/failed default pending+CHECK, attempt_count Integer NOT NULL default 0 CHECK>=0, next_attempt_at/processed_at UTC_TS nullable, last_error String(2048) nullable, created_at UTC_TS NOT NULL; UNIQUE(service_id,requested_generation,operation_kind) — the worker-task dedup key, all-NULL/partial-NULL rows NOT deduped on either dialect). Alembic revision `m1_model_008_audit_outbox` extends from `m1_model_007_permission_key`; `make test-db-contract TEST=models_audit_outbox` → 60 passed on BOTH real dialects via fresh-DB `upgrade head`. Core contract: business change + audit_event + outbox commit/roll back atomically in one transaction (`test_audit_and_outbox_commit_atomically`). Previous: `M1-MODEL-007` (权限与 API Key 元数据模型; `mcp_service_permission` per §5.12 + `api_key` per §5.13; `make test-db-contract TEST=models_permission_key` → 34 passed; four controller-approved query indexes; API key plaintext never persisted — application-layer); `M1-MODEL-006` Operation 与 Runtime Condition 模型; `M1-MODEL-005` Artifact 与 Build 模型; `M1-MODEL-004` Toolset 与 Tool 模型; `M1-MODEL-003` 配置 Revision 模型; `M1-MODEL-002` 服务模型; `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
