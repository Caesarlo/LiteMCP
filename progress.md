...
- Last passing feature: `M1-MODEL-003` (配置 Revision 模型; `backend/src/litemcp/db/models.py` gains `ServiceConfigRevision` on the shared `Base` — `service_config_revision` holds immutable desired-config revisions per 01-data-model §5.3: content fields `public_config` JSON_DOC NOT NULL / `secret_blob_id` ID-nullable WITHOUT FK (target `service_secret` is a later table) / `config_digest` String(64), lifecycle fields `state` (draft/validating/validated/active/rejected/superseded) / `validation_report` / `activated_at` / `superseded_at`, `service_id` FK→mcp_service RESTRICT, UNIQUE `(service_id,generation)` + UNIQUE `(id,service_id)` backing the §5.2 L165 composite active-pointer FK, generic create fields ONLY (no `updated_*`/`row_version` — immutable, §5.3 L188), `created_by` String(128) per uniform §3.2; Alembic revision `m1_model_003_config_revision` extends the graph from `m1_model_002_service`; `make test-db-contract TEST=models_config_revision` → 20 passed on BOTH real dialects via fresh-DB `upgrade head`. Supporting change to the shared type surface (controller-approved, recorded in Session 019 checkpoint): `JSON_DOC` now defaults `none_as_null=True` so top-level Python None binds SQL NULL and a NOT NULL JSON column is DB-enforced (§3.2 L63). Previous: `M1-MODEL-002` 服务模型; `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
