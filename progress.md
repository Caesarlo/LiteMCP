...
### Session 028 · 2026-08-13

#### Checkpoint 75 · Status check: baseline healthy, M2 next

- Feature: none (status/planning only).
- Result: Startup workflow re-run per AGENTS.md. `node scripts/validate-feature-list.js` exits 0 (56 features / 9 milestones: 32 passing, 0 in_progress, 0 blocked, 24 not_started). Git clean on `main`; `core.hooksPath` = `.githooks`; no ahead/behind vs `origin/main` (all commits pushed, incl. `dc0d0c6` M2 decomposition). M1 complete: 17/17 passing. The 24 not_started = 18 decomposed M2 features (M2-AUTH-001..010, M2-WEB-001, M2-RBAC-001..002, M2-STEPUP-001, M2-FE-001..004) + 6 M3–M8 scope placeholders.
- Baseline verification: non-database backend suite (`pytest -q --ignore=tests/db`) → 61 passed. The 7 storage-contract errors are the documented sandbox temp-dir limitation (pytest `tmp_path` fixture → `PermissionError: [WinError 5]`; `--basetemp` and `TMP`/`TEMP` redirects do not bypass it in this session; `git status` also warns it cannot read `.pytest-tmp/`). Same class as Checkpoint 58 — environmental, not a code regression; full-suite runs require the sandbox-external pytest path. DB gates (`make test-db-contract TEST=...`) need Docker services up.
- Files changed: `progress.md` (Current Verified State snapshot refresh + this checkpoint).
- Evidence: validator 0-exit; `git status -sb` clean/pushed; 61 passed + 7 environmental errors on the non-db suite.
- Next action: activate `M2-AUTH-001` (实现首个管理员初始化, priority 201) — highest-priority dependency-ready feature. Isolated TDD per AGENTS.md: test-writer → `backend/tests/auth/test_bootstrap.py` (behavior/verification/source_refs only), controller RED, fresh implementer, controller GREEN, gate `cd backend && uv run pytest tests/auth/test_bootstrap.py`.

### Session 029 · 2026-08-18

#### Checkpoint 76 · M2-AUTH-001 activated

- Feature: `M2-AUTH-001` (实现首个管理员初始化).
- Status change: `not_started` → `in_progress`.
- Contract (docs/architecture/02-admin-auth.md §4.1): an empty database can only get its first global admin via a controlled bootstrap flow — DB transaction locks initialization state, confirms no user exists, then creates the first `role=admin,status=active` user; two concurrent initializations yield exactly one success; password never appears in CLI args, shell history, or logs; after init there is no reusable bootstrap HTTP endpoint or default account.
- Verification: `node scripts/validate-feature-list.js` passed before activation (32 passing, 1 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_bootstrap.py`.

#### Checkpoint 77 · M2-AUTH-001 passed

- Feature: `M2-AUTH-001` (实现首个管理员初始化).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent (isolated context, given only behavior/verification/§4.1 source text) wrote `backend/tests/auth/test_bootstrap.py` against a designed API surface `litemcp.auth.bootstrap.bootstrap_first_admin(session, *, username, password) -> User` / `AlreadyInitializedError`, using the existing live-Postgres/MySQL fixture convention from `tests/db/test_soft_delete.py`. Controller RED confirmed: all 8 parametrized tests failed with `ModuleNotFoundError: No module named 'litemcp.auth'` (the missing module, not a typo) — required restarting Docker Desktop mid-session first (it crashed with an `inference manager` file-permission error on the user's machine; user manually retried and it came up healthy, no code involved).
  2. Fresh implementer subagent (test file + behavior only, no test-writer reasoning) added `backend/src/litemcp/auth/bootstrap.py` and a new migration `backend/migrations/versions/m2_auth_001_bootstrap_lock.py` creating a dedicated single-row `bootstrap_lock` table. `bootstrap_first_admin` runs inside the caller's already-open transaction (never calls `session.begin()`/`commit()` itself), takes `SELECT id FROM bootstrap_lock WHERE id = 1 FOR UPDATE` before counting `user` rows — serializing concurrent bootstrap attempts across sessions/processes/dialects via a real DB row lock (a plain count-then-insert or locking the possibly-empty `user` table directly were both rejected as unsafe) — then either raises `AlreadyInitializedError` or creates the admin with an Argon2id-hashed password (`argon2.PasswordHasher()`, no pre-existing hashing helper found) and an inline NFKC+trim+casefold username normalizer (none pre-existed). `session.expunge()` after `flush()` keeps the returned `User`'s attributes readable after the caller's transaction later commits/closes.
  3. **Controller-diagnosed test-fixture bug (not an implementation defect):** implementer reported 6/8 passing, with the 2 `already_exists` tests failing on `DetachedInstanceError` reading `existing.id`. Reproduced independently: `_seed_existing_user`'s `AsyncSession(engine)` used the SQLAlchemy default `expire_on_commit=True`, so after its `async with` block closed, `existing.id` was both expired and detached — exactly the failure mode `backend/src/litemcp/db/session.py`'s own docstring calls out and works around with `expire_on_commit=False`. Fixed the one-line fixture (not any assertion) to match that project convention; reran — 8/8 passed. `ruff check` then flagged 5 pre-existing cosmetic issues in the test file (unused `Base` import, import sort, 3× mergeable nested `with`); auto-fixed with `ruff check --fix`, no behavior change, reran green.
- Files changed: `backend/src/litemcp/auth/__init__.py`, `backend/src/litemcp/auth/bootstrap.py`, `backend/migrations/versions/m2_auth_001_bootstrap_lock.py`, `backend/tests/auth/test_bootstrap.py` (test-writer + controller fixture/lint fix), `feature_list.json` (status → passing + evidence + implementation files), `progress.md`.
- Verification: `cd backend && .venv/Scripts/python.exe -m pytest tests/auth/test_bootstrap.py -q` → 8 passed (postgres+mysql × 4 tests, incl. an `asyncio.gather` concurrency race asserting exactly one winner). `ruff check src tests` → clean. `mypy src` → no issues (18 files). `alembic heads` → single head `m2_auth_001_bootstrap_lock`. Regression: `pytest -q --ignore=tests/db --ignore=tests/storage` → 69 passed; `pytest tests/db -q` → 247 passed (full migration-graph + dialect regression, no failures). `node scripts/validate-feature-list.js` → 56 features, 33 passing, 0 in_progress, 0 blocked.
- Scope note: this slice delivers only the DB-layer bootstrap function per the controlling session's plan — no `litemcp admin create` CLI and no HTTP endpoint were added (consistent with §4.1's "no reusable bootstrap HTTP endpoint" requirement; CLI wiring, if tracked separately, is not yet a feature entry — flagged as a possible gap, not silently assumed done).
- Next action: commit the implementation, then select the next dependency-ready M2 feature (e.g. `M2-AUTH-002`, 实现 Argon2id 密码存储) via the same isolated TDD workflow.

#### Checkpoint 78 · M2-AUTH-002 activated

- Feature: `M2-AUTH-002` (实现 Argon2id 密码存储).
- Status change: `not_started` → `in_progress`.
- Contract (docs/architecture/02-admin-auth.md §5.2, scoped narrowly): new passwords hashed with configurable Argon2id parameters meeting the baseline floor (`m>=19456 KiB, t>=2, p>=1`), each with an independent random salt, stored as a self-describing PHC string; verification takes a uniform failure path so a caller cannot distinguish *why* verification failed from the exception type. `needs_rehash`-on-login, bcrypt migration, and the pepper/Secret-Manager requirement are explicitly out of scope (tracked as `M2-AUTH-003`/`M2-AUTH-004`).
- Verification: `node scripts/validate-feature-list.js` passed before activation (33 passing, 1 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_password_argon2.py`.

#### Checkpoint 79 · M2-AUTH-002 passed

- Feature: `M2-AUTH-002` (实现 Argon2id 密码存储).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent (isolated context, only the §5.2 excerpt + explicit out-of-scope list) designed `litemcp.auth.password` (`hash_password`, `verify_password`, `Argon2Parameters`, `PasswordVerificationError`) and wrote `backend/tests/auth/test_password_argon2.py` — 17 DB-free unit tests covering PHC format, the baseline parameter floor, independent-salt-per-call, configurable-override reflection, and a uniform `PasswordVerificationError` across wrong-password/malformed-hash/wrong-algorithm-hash/empty-string. Controller RED confirmed: collection failure `ModuleNotFoundError: No module named 'litemcp.auth.password'` — the missing module, not a typo.
  2. Fresh implementer subagent (test file + behavior only) added `backend/src/litemcp/auth/password.py` and three new `Settings` fields (`argon2_memory_cost_kib`/`argon2_time_cost`/`argon2_parallelism`, `LITEMCP_ARGON2_*`) in `backend/src/litemcp/core/config.py` so the no-override hashing path reads real deployment configuration (falling back to the hardcoded baseline only when `Settings()` construction itself fails, e.g. in DB-free unit tests with no `.env`/`LITEMCP_DATABASE_URL` — verified this fallback is genuinely exercised, not dead code, since `backend/` has no local `.env`). `verify_password` catches `argon2.exceptions.VerificationError`/`InvalidHashError`/`InvalidHash` and re-raises uniformly as `PasswordVerificationError`. Left `bootstrap.py`'s inline `argon2.PasswordHasher()` untouched (adjudicated: cosmetic consistency versus risk to the already-passing `M2-AUTH-001` regression — not worth it this slice; flagged as a follow-up, not a defect).
- Files changed: `backend/src/litemcp/auth/password.py` (new), `backend/src/litemcp/core/config.py` (implementer), `backend/tests/auth/test_password_argon2.py` (test-writer), `feature_list.json` (status → passing + evidence + implementation files), `progress.md`.
- Verification: `cd backend && .venv/Scripts/python.exe -m pytest tests/auth/test_password_argon2.py -q` → 17 passed. `ruff check src tests` → clean. `mypy src` → no issues (19 files). Regression: `pytest tests/auth/test_bootstrap.py -q` → 8 passed (sibling M2-AUTH-001 unaffected); `pytest -q --ignore=tests/db --ignore=tests/storage` → 86 passed. `node scripts/validate-feature-list.js` → 56 features, 34 passing, 0 in_progress, 0 blocked.
- Scope note: `bootstrap.py` still hashes inline rather than calling the new module — a recorded consistency follow-up, not a functional gap (both paths meet the Argon2id baseline).
- Next action: commit, then select the next dependency-ready M2 feature (`M2-AUTH-003`, 升级兼容的 bcrypt 密码) via the same isolated TDD workflow.

#### Checkpoint 80 · M2-AUTH-003 activated

- Feature: `M2-AUTH-003` (升级兼容的 bcrypt 密码).
- Status change: `not_started` → `in_progress`.
- Contract (docs/architecture/02-admin-auth.md §5.2): migrating an existing bcrypt hash upgrades it to Argon2id immediately upon successful verification; bcrypt is legacy-compat only, never the default for new passwords. Scoped to a DB-layer function only (no login/HTTP endpoint exists yet, matching the M2-AUTH-001/002 precedent).
- Supporting change: added `bcrypt` (v5.0.0) as a project dependency via `uv add bcrypt` (no bcrypt-verification library previously present).
- Verification: `node scripts/validate-feature-list.js` passed before activation (34 passing, 1 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_password_migration.py`.

#### Checkpoint 81 · M2-AUTH-003 passed

- Feature: `M2-AUTH-003` (升级兼容的 bcrypt 密码).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent (isolated context; given only the §5.2 excerpt, explicit out-of-scope list, and read-only access to the passing `bootstrap.py`/`password.py` siblings plus `test_bootstrap.py`'s live-DB fixture convention) designed `litemcp.auth.password_migration.verify_and_migrate_password(session, user, password) -> bool` and wrote `backend/tests/auth/test_password_migration.py` — 5 tests × 2 dialects, seeding real bcrypt hashes via the `bcrypt` library, asserting exact byte-for-byte digest equality on every failure path (not just "still bcrypt-shaped"). Controller RED confirmed: 10 setup errors, `AssertionError: M2-AUTH-003 must expose litemcp.auth.password_migration.verify_and_migrate_password` — the missing module, not a typo.
  2. Fresh implementer subagent (test file + behavior only) added `backend/src/litemcp/auth/password_migration.py`: detects bcrypt (`$2a$`/`$2b$`/`$2y$` prefix) vs. Argon2id (`$argon2id$` prefix) from `user.password_hash`; Argon2id path delegates straight to M2-AUTH-002's `verify_password` with no DB write; bcrypt path verifies via `bcrypt.checkpw`, and only on success mutates `user.password_hash = hash_password(password)` in place (relying on SQLAlchemy's unit-of-work plus the caller's already-open transaction, no internal `begin()`/`commit()`, matching `bootstrap.py`'s convention) — on failure it raises before any mutation, so the stored digest is provably untouched.
  3. **Controller lint tightening (not a behavior change):** the test file used blind `pytest.raises(Exception)` (ruff `B017`) in three places; since the module now demonstrably and consistently raises `PasswordVerificationError`, tightened those three assertions to the specific exception type and re-ran green. Also auto-fixed one cosmetic import-wrap `ruff --fix` finding.
- Files changed: `backend/src/litemcp/auth/password_migration.py` (new), `backend/pyproject.toml`/`uv.lock` (bcrypt dependency), `backend/tests/auth/test_password_migration.py` (test-writer + controller lint tightening), `feature_list.json` (status → passing + evidence + implementation files), `progress.md`.
- Verification: `cd backend && .venv/Scripts/python.exe -m pytest tests/auth/test_password_migration.py -q` → 10 passed (postgres+mysql × 5). `ruff check src tests` → clean. `mypy src` → no issues (20 files). `alembic heads` → single head `m2_auth_001_bootstrap_lock` (no new migration needed — `password_hash` column reused as-is). Regression: `pytest tests/auth -q` → 35 passed (bootstrap + argon2 + migration); `pytest -q --ignore=tests/db --ignore=tests/storage` → 96 passed. `node scripts/validate-feature-list.js` → 56 features, 35 passing, 0 in_progress, 0 blocked.
- Next action: commit, then select the next dependency-ready M2 feature (`M2-AUTH-004`, 实现登录成功契约, or whichever `not_started` M2 entry has all dependencies `passing`) via the same isolated TDD workflow.

#### Checkpoint 82 · M2-AUTH-004 activated

- Feature: `M2-AUTH-004` (实现登录成功契约).
- Status change: `not_started` → `in_progress`.
- Contract (docs/architecture/02-admin-auth.md §7.1/§7.2/§8.1/§8.3, scoped narrowly): valid credentials return a short-lived Access JWT with a fixed header/claims shape (`alg=HS256`, `kid`, `typ=at+jwt`; `iss/aud/sub/sid/jti/iat/nbf/exp/token_type`) and establish a Redis-backed Refresh Session (opaque `session_id.random_secret` token, only `SHA-256(random_secret)` ever stored, idle/absolute TTL, membership in a `user_sessions` set for future "log out everywhere"). Scoped to the DB/session layer only — no HTTP login endpoint, no password verification (already built), no lockout/JWT-verification/refresh-rotation/replay-detection/logout (all separate M2-AUTH-005..010 feature IDs).
- Supporting change: added `pyjwt[crypto]` as an explicit project dependency (`uv add "pyjwt[crypto]"`; was previously only a transitive dep).
- Verification: `node scripts/validate-feature-list.js` passed before activation (35 passing, 1 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_login.py`.

#### Checkpoint 83 · M2-AUTH-004 passed

- Feature: `M2-AUTH-004` (实现登录成功契约).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent (isolated context; given the verbatim §7.1/§7.2/§8.1/§8.3 excerpts, explicit out-of-scope list, and read-only access to the passing sibling auth modules for style precedent) designed `litemcp.auth.session.create_login_session(redis_client, user) -> LoginSession` and wrote `backend/tests/auth/test_login.py` — 8 tests against a live Redis instance (no Postgres/MySQL needed, since the function only reads `user.id`), each test run isolated via a per-test UUID-suffixed Redis key namespace rather than a global `FLUSHDB`.
  2. **Controller-caught test bug before dispatching the implementer:** `test_access_ttl_reads_overridden_setting_not_hardcoded` only requested the `monkeypatch`/`environment_name` fixtures, omitting the `admin_auth_settings` fixture that sets the required `LITEMCP_DATABASE_URL`/`ENCRYPTION_KEYS`/`ADMIN_JWT_SECRET`/etc. env vars — once the target module existed, `Settings()` construction would fail for reasons unrelated to the feature. Fixed by adding the missing fixture parameter (one line) before dispatching the implementer, so the RED signal stayed clean. Controller RED confirmed afterward: all 8 tests failed on `ModuleNotFoundError: No module named 'litemcp.auth.session'` (7 as fixture-setup errors, 1 — the just-fixed test — as the expected `AssertionError` wrapping the same import failure) — the missing module, not a typo.
  3. Fresh implementer subagent (test file + behavior only) added `backend/src/litemcp/auth/session.py`: `session_id`/`random_secret` via `secrets.token_urlsafe` (128-bit / 256-bit), JWT built with PyJWT (`iat=nbf`, `exp=iat+admin_access_ttl_seconds`, unique `jti`/`kid` per call), and one Redis pipeline per login (`HSET` the session hash, `EXPIRE` to the absolute TTL, `SADD` into the user's session set) so two concurrent logins never race each other. Added seven new `Settings` fields (`admin_jwt_secret`, `admin_jwt_issuer`, `admin_jwt_audience`, `admin_access_ttl_seconds`, `admin_refresh_idle_ttl_seconds`, `admin_refresh_absolute_ttl_seconds`, `admin_session_environment`) in `backend/src/litemcp/core/config.py`. **Design adjudication surfaced in the implementer's own report, reviewed by the controller:** making `admin_jwt_secret`/`admin_jwt_issuer` hard-required (no default, like `database_url`) broke most of `tests/core/test_config.py` plus two `tests/db` files. Instead of a required field, the implementer gave both an obviously-fake placeholder default and extended the *existing* `_refuse_unsafe_production` validator (the same one already guarding `encryption_keys`) to reject either placeholder when `environment=="prod"` — preserving the docs' "production must configure a real secret" intent without a required field breaking dev/test construction everywhere. Controller reviewed this diff directly (not just the report) and confirmed it reuses the existing validator rather than duplicating logic, and that the one `tests/core/test_config.py` fixture change (`_env()` helper gains two real, non-placeholder env vars) is a narrow, legitimate fixture fix — not a weakened assertion.
  4. **Controller lint cleanup:** ruff flagged one `BLE001` (blind `except Exception`) in the test file's Redis-reachability skip guard — an intentional catch-all ("any connection failure → skip, not RED"), so silenced with a targeted `# noqa: BLE001` rather than narrowing the catch.
...
### Session 032 · 2026-08-19

#### Checkpoint 87 · M2-AUTH-006 activated

- Feature: `M2-AUTH-006` (实现 Access JWT 校验).
- Status change: `not_started` → `in_progress`.
- Contract (`docs/architecture/02-admin-auth.md` §7.2/§7.3/§11/§18, scoped to the application-layer verifier, matching AUTH-001..005): reject `alg=none`, algorithms outside the server allowlist (`ADMIN_JWT_ALGORITHM` default HS256), unknown `kid`, wrong `typ` (`at+jwt`) or `token_type` (`access`); verify signature, `iss`, `aud` (default `litemcp-admin-api`), `sub` UUID format, `iat/nbf/exp` with clock skew at most 30s (`ADMIN_JWT_CLOCK_SKEW_SECONDS`); reject duplicate/wrong-type security claims, oversized or unparseable tokens. Load user by `sub`; require `status=active` (`disabled`/`locked`/missing fail). `iat < password_changed_at` → `TOKEN_STALE`. Auth context `role` comes from the DB row, never from JWT. DB unavailable fail-closed. Support verifying with current `kid` plus not-yet-expired rotation keys. Out of scope: HTTP `get_current_user`/admin routers, refresh rotation, jti denylist, audit persistence, CSRF.
- Verification: `node scripts/validate-feature-list.js` passed before activation (37 passing, 0 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_access_jwt.py`.

#### Checkpoint 88 · M2-AUTH-006 passed

- Feature: `M2-AUTH-006` (实现 Access JWT 校验).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent produced `backend/tests/auth/test_access_jwt.py` pinning `litemcp.auth.access_jwt.verify_access_jwt` / `AccessJwtError` / `AccessAuthContext`. Controller RED: `49 failed` — `ModuleNotFoundError: No module named 'litemcp.auth.access_jwt'` — the missing module, not a typo.
  2. Fresh implementer added `backend/src/litemcp/auth/access_jwt.py` plus Settings `admin_jwt_kid` / `admin_jwt_algorithm` / `admin_jwt_clock_skew_seconds` / `admin_jwt_previous_keys`. Login issuance in `session.py` now stamps the configured `kid` so issued tokens can be verified. Controller lint-only test cleanup: `finally` early-return (ruff B012) and a non-interpolating f-string (F541); assertions unchanged.
- Files changed: `backend/tests/auth/test_access_jwt.py` (test-writer + ruff lint), `backend/src/litemcp/auth/access_jwt.py` (new), `backend/src/litemcp/core/config.py`, `backend/src/litemcp/auth/session.py` (implementer), `feature_list.json`, `progress.md`.
- Verification: controller GREEN `pytest tests/auth/test_access_jwt.py -q` → 49 passed. `ruff check src tests` clean. `mypy src` no issues (23 files). `alembic heads` → single head `m2_auth_001_bootstrap_lock` (no new migration). Regression: `pytest tests/core tests/auth -q` → 134 passed; `pytest -q --ignore=tests/db --ignore=tests/storage` → 178 passed. Full `tests/db` suite was not re-run (no schema/migration change; live JWT tests already upgrade both dialects to head).
- Scope note: HTTP `get_current_user`, refresh rotation, jti denylist, audit events, and CSRF remain out of scope.
- Next action: commit when requested, then `M2-AUTH-007` (实现 Refresh Token 轮换) via the same isolated TDD workflow.

### Session 033 · 2026-08-19

#### Checkpoint 89 · M2-AUTH-007 activated

- Feature: `M2-AUTH-007` (实现 Refresh Token 轮换).
- Status change: `not_started` → `in_progress`.
- Baseline: `M2-AUTH-004` is passing (`create_login_session` issues Access JWT + Redis refresh session). No `rotate`/`refresh` application-layer function and no `tests/auth/test_refresh_rotation.py`.
- Contract (`docs/architecture/02-admin-auth.md` §8, scoped to application-layer rotation like AUTH-001..006 — no HTTP cookie/CSRF router): parse opaque `<session_id>.<random_secret>`; Redis Lua or equivalent atomic txn: load session; reject missing/idle/absolute expired; constant-time compare secret hash to `current_secret_hash`; on match, generate a new secret and atomically replace hash / `last_refreshed_at` / idle expiry (idle extends, never past absolute expiry); user must be `active` and session `created_at` must not predate `password_changed_at`; return a new access JWT and new refresh token; no grace window for the old token (old token immediately fails). Out of scope: HTTP `POST /auth/refresh` + Set-Cookie/CSRF (`M2-WEB-001`), replay-family revoke + `auth.refresh_reuse_detected` audit (`M2-AUTH-008`), logout (`M2-AUTH-009`), Redis fail-closed observability (`M2-AUTH-010`).
- Verification: `node scripts/validate-feature-list.js` passed before activation (38 passing, 0 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_refresh_rotation.py`.

#### Checkpoint 90 · M2-AUTH-007 passed

- Feature: `M2-AUTH-007` (实现 Refresh Token 轮换).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent produced `backend/tests/auth/test_refresh_rotation.py` pinning `litemcp.auth.refresh.rotate_refresh_token` / `RefreshRotation` / `compare_refresh_secret` / the `RefreshRejected` family. Controller RED: collection `ModuleNotFoundError: No module named 'litemcp.auth.refresh'` — the missing module, not a typo.
  2. Fresh implementer added `backend/src/litemcp/auth/refresh.py`. Successful rotate writes a new SHA-256 secret hash + idle timestamps, then confirms ownership before minting via existing `_build_access_token`. Old secret fails immediately (no grace window). Family deletion on mismatch is deferred to `M2-AUTH-008`. Supporting change: `backend/src/litemcp/db/types.py` uses MySQL physical types only when `dialect.name == "mysql"` so SQLite `create_all` in this suite works; PostgreSQL and MySQL mappings are unchanged. Implementer lint-only test cleanup (import order / unused unpack); assertions unchanged.
- Files changed: `backend/tests/auth/test_refresh_rotation.py` (test-writer + ruff lint), `backend/src/litemcp/auth/refresh.py` (new), `backend/src/litemcp/db/types.py` (implementer), `feature_list.json`, `progress.md`.
- Verification: controller GREEN `pytest tests/auth/test_refresh_rotation.py -q` → 20 passed. `ruff check src tests` clean. `mypy src` no issues (24 files). `alembic heads` → single head `m2_auth_001_bootstrap_lock` (no new migration). Regression: `pytest tests/core tests/auth -q` → 154 passed; `pytest -q --ignore=tests/db --ignore=tests/storage` → 198 passed; `pytest tests/db/test_types.py -q` → 6 passed.
- Scope note: HTTP `POST /auth/refresh` + cookies/CSRF, replay-family revoke + `auth.refresh_reuse_detected` audit, logout, and Redis fail-closed remain out of scope.
- Next action: commit when requested, then `M2-AUTH-008` (检测 Refresh Token 重放) via the same isolated TDD workflow.

### Session 034 · 2026-08-19

#### Checkpoint 91 · M2-AUTH-008 activated

- Feature: `M2-AUTH-008` (检测 Refresh Token 重放).
- Status change: `not_started` → `in_progress`.
- Baseline: `M2-AUTH-007` is passing (`rotate_refresh_token` atomically replaces the Redis secret hash; mismatch does not yet delete the family or write audit). No `tests/auth/test_refresh_replay.py`.
- Contract (`docs/architecture/02-admin-auth.md` §8.3/§8.4/§11/§17 + `01-data-model.md` §5.14, scoped to application-layer like AUTH-001..007 — no HTTP cookie/CSRF router): after a successful rotation, presenting the retired refresh token (or a session-id-matching forged secret) is replay/forgery. Constant-time compare against `current_secret_hash` fails → delete the entire Redis session (token family) and remove it from `user_sessions`; persist `audit_event` with `action=auth.refresh_reuse_detected`; do not issue Access JWT or a new refresh token. Missing/expired sessions stay ordinary rejects (not reuse audit). No old-token grace window. Audit must not contain plaintext tokens, secrets, cookies, or Authorization headers. Out of scope: HTTP `POST /auth/refresh` (`M2-WEB-001`), logout (`M2-AUTH-009`), Redis fail-closed observability (`M2-AUTH-010`).
- Verification: `node scripts/validate-feature-list.js` passed before activation (39 passing, 0 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_refresh_replay.py`.

#### Checkpoint 92 · M2-AUTH-008 passed

- Feature: `M2-AUTH-008` (检测 Refresh Token 重放).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent produced `backend/tests/auth/test_refresh_replay.py` pinning replay/forgery family revoke + `auth.refresh_reuse_detected` via `rotate_refresh_token`. Controller RED: `4 failed, 6 passed` — Redis `admin_session` still existed after mismatch (`exists==1`); unknown/expired already rejected without reuse audit — the missing family-revoke/audit path, not a typo.
  2. Fresh implementer extended `backend/src/litemcp/auth/refresh.py`: on live-session secret mismatch, delete `admin_session`, `srem` the id from `user_sessions`, persist `AuditEvent` (`action=auth.refresh_reuse_detected`, `result=denied`, `resource_id=session_id`), then raise `RefreshSecretMismatch`. Missing/idle/absolute-expired paths unchanged.
  3. Controller supporting change: AUTH-007 `MemoryRedis` gained `srem`/`sadd`; `test_old_refresh_token_fails_immediately_with_no_grace_window` now expects the successor token to fail with `RefreshSessionMissing` after replay; `test_wrong_secret_fails_closed_without_minting_tokens` now asserts the session key is gone rather than hash-unchanged.
- Files changed: `backend/tests/auth/test_refresh_replay.py` (test-writer), `backend/src/litemcp/auth/refresh.py` (implementer), `backend/tests/auth/test_refresh_rotation.py` (controller supporting), `feature_list.json`, `progress.md`.
- Verification: controller GREEN `pytest tests/auth/test_refresh_replay.py -q` → 10 passed. `ruff check src tests` clean. `mypy src` no issues (24 files). `alembic heads` → single head `m2_auth_001_bootstrap_lock`. Regression: `pytest tests/core tests/auth -q` → 164 passed; `pytest -q --ignore=tests/db --ignore=tests/storage` → 208 passed.
- Scope note: HTTP `POST /auth/refresh` + cookies/CSRF, logout (`M2-AUTH-009`), and Redis fail-closed remain out of scope.
- Next action: commit when requested, then `M2-AUTH-009` (实现登出与会话撤销) via the same isolated TDD workflow.

### Session 035 · 2026-08-19

#### Checkpoint 93 · M2-AUTH-009 activated

- Feature: `M2-AUTH-009` (实现登出与会话撤销).
- Status change: `not_started` → `in_progress`.
- Baseline: `M2-AUTH-007` is passing (rotation of a live session). `M2-AUTH-008` is passing (replay revokes the family). No `tests/auth/test_logout_revoke.py`. No application-layer current-session / all-sessions logout API yet.
- Contract (`docs/architecture/02-admin-auth.md` §8.3/§11/§13/§17, scoped to application-layer like AUTH-001..008 — no HTTP cookie/CSRF router): user can revoke the current Refresh Session or all Refresh Sessions for that user. Ordinary logout deletes the current Redis `admin_session` and removes it from `user_sessions`. Logout-all deletes every session in that user's `user_sessions` set. After either revoke, a subsequent refresh of a deleted session must fail. Persist `audit_event` with `action=auth.logout` (current) or `auth.logout_all` (all devices). Do not denylist access JWT `jti` (first version: leaked access may live until `exp`). Audit must not contain plaintext tokens, secrets, cookies, or Authorization headers. Out of scope: HTTP `POST /auth/logout` and `POST /auth/logout-all` + Set-Cookie/CSRF (`M2-WEB-001`), Redis fail-closed observability (`M2-AUTH-010`), password-change revoke-all (later change-password feature).
- Verification: `node scripts/validate-feature-list.js` passed before activation (40 passing, 0 in_progress, 0 blocked).
- Next action: dispatch isolated test-writer for `backend/tests/auth/test_logout_revoke.py`.

#### Checkpoint 94 · M2-AUTH-009 passed

- Feature: `M2-AUTH-009` (实现登出与会话撤销).
- Status change: `in_progress` → `passing`.
- Result (isolated TDD split per AGENTS.md):
  1. Test-writer subagent produced `backend/tests/auth/test_logout_revoke.py` pinning `litemcp.auth.logout` (`logout_current_session`, `logout_all_sessions`, `LogoutRejected`) plus post-revoke `litemcp.auth.refresh.refresh_session`. Controller RED: `11 failed` — `ModuleNotFoundError: No module named 'litemcp.auth.logout'` — the missing module, not a typo.
  2. Fresh implementer added `backend/src/litemcp/auth/logout.py`. Current logout constant-time compares the secret, deletes Redis `admin_session`, `srem` from `user_sessions`, and inserts `audit_event` (`action=auth.logout`, `result=success`). Logout-all deletes every listed session plus the set key and writes `auth.logout_all`. Wrong/missing current session raises `LogoutRejected` without a success audit. Optional `user_agent`/`cookie_header`/`authorization` kwargs are accepted and discarded so they cannot land in audit. Supporting: `refresh_session` on FakeRedis JSON sessions (does not mint tokens; production rotation remains `rotate_refresh_token`). Implementer lint-only test cleanup (`UTC` alias); assertions unchanged.
- Files changed: `backend/tests/auth/test_logout_revoke.py` (test-writer + ruff lint), `backend/src/litemcp/auth/logout.py` (new), `backend/src/litemcp/auth/refresh.py` (implementer), `feature_list.json`, `progress.md`.
