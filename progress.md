...
- Last passing feature: `M1-MODEL-002` (服务模型; `backend/src/litemcp/db/models.py` gains `Service` on the shared `Base` — `mcp_service` holds stable identity + desired state + publish pointers + runtime summary per 01-data-model §5.2, with portable `litemcp.db.types` columns, UNIQUE `(namespace_key, name_normalized, uniqueness_scope)`, three market/ownership indexes, portable CHECKs (type/desired_status/runtime_status/agent_auth_mode enums, `observed_generation <= generation`, all four stdio-only columns NULL for non-stdio), FK `team.id` RESTRICT, §3.2 audit + §3.3 soft-delete fields; Alembic revision `m1_model_002_service` extends the graph from `m1_model_001_user_team`; `make test-db-contract TEST=models_service` → 20 passed on BOTH real dialects via fresh-DB `upgrade head`. The two active-version pointers `active_config_revision_id`/`active_toolset_id` are pinned present + nullable + ID-typed WITHOUT FK (their targets `service_config_revision`/`toolset` are later features). Controller adjudications recorded in Session 018 checkpoint: audit/soft-delete identity columns (`created_by`/`updated_by`/`deleted_by`) aligned to String(128) per the uniform §3.2 convention shared by user/team/team_membership (test-writer had pinned ID kind), and the stdio-only CHECK strengthened from `queue_max_depth` alone to all four stdio-only columns per §5.2 L163. Previous: `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
