...
- Last passing feature: `M1-MODEL-001` (用户与团队模型; `backend/src/litemcp/db/models.py` — shared `Base` + `User`/`Team`/`TeamMembership` using only `litemcp.db.types` logical types with DB-level UNIQUE/CHECK/FK per 01-data-model §5.1/§5.16/§5.17; Alembic revision `m1_model_001_user_team` extends the graph from the bootstrap root; `make test-db-contract TEST=suite_name` gate. `make test-db-contract TEST=models_user_team` → 13 passed on BOTH real dialects via fresh-DB `upgrade head` (offline structural contract + live CRUD/UNIQUE/CHECK/FK/lifecycle). Supporting changes adjudicated: `session.py` gains `expire_on_commit=False` (committed attributes readable after session release; M1-DB-001 contract intact, full suite 73 passed) and `TeamMembership` declares `relationship()` for flush ordering. The implementer's `make help` line broke cmd (literal `<` = input redirect) — caught by the M0-CMD-001 gate, fixed to `TEST=suite_name`, gate 7/7 in both branches. Previous: `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
