...
- Current blocker: None. Docker Desktop is running; compose `database`/`mysql`/`redis` are healthy (host ports 5433/3307/6379). M1 milestone complete (17/17 passing); M2 in progress: 7/18 passing.
- Last passing feature: `M2-AUTH-007` (实现 Refresh Token 轮换; `litemcp.auth.refresh.rotate_refresh_token` atomically replaces the Redis secret hash and mints a new Access JWT — application-layer only, no HTTP refresh route).
- Next best step: activate `M2-AUTH-008` (检测 Refresh Token 重放, priority 208, dependency `M2-AUTH-007` passing). Isolated TDD: test-writer for `backend/tests/auth/test_refresh_replay.py`, controller RED, fresh implementer, controller GREEN, then the declared gate plus auth/core regressions.
