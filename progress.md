...
- Last passing feature: `M1-DB-002` (cross-dialect base types; `litemcp/db/types.py` six TypeDecorators `ID/UTC_TS/JSON_DOC/CIPHERTEXT/LONG_TEXT/ENUM_CODE` with per-dialect `load_dialect_impl` — PG UUID/TIMESTAMPTZ/JSONB/BYTEA/TEXT, MySQL CHAR(36)/DATETIME(6)/JSON/LONGBLOB/LONGTEXT, VARCHAR+CHECK enums. `make test-db-types` → 6 tests against REAL PostgreSQL 16.14 (localhost:5433) + MySQL 8.4.11 (localhost:3307): DDL contract, live round-trips (UUID, UTC instant incl. MySQL naive-DATETIME UTC semantics, JSON, ciphertext bytes, >4KiB text, enum), DB-level enum rejection on both. First feature verified against real two-dialect DBs; Docker compose (M0-BOOT-001) + `mysql` dialects profile. Previous: `M1-DB-001` async session factory; `M0-CI-001` ci-fast gate; `M0-CONTRACT-001` OpenAPI snapshot gate).
