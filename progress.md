...
- Last passing feature: `M1-MODEL-004` (Toolset 与 Tool 模型; `backend/src/litemcp/db/models.py` gains `Toolset`/`McpTool` on the shared `Base` — `toolset` holds the atomic publish unit per 01-data-model §5.7 (service_id FK→mcp_service RESTRICT, config_revision_id FK→`service_config_revision` nullable [first real FK landed], version_no BigInteger UNIQUE(service_id,version_no), source_kind/state ENUM_CODE+CHECK, source_digest String(64), mcp_protocol_version String(16), json_schema_dialect String(128) server_default 2020-12 URI, server_capabilities/server_info/validation_report JSON_DOC, instructions LONG_TEXT, tool_count Integer CHECK>=0, activated_at/retired_at UTC_TS, full §3.2 audit, UNIQUE(id,service_id)); `mcp_tool` stores lossless MCP Tool definitions per §5.8 (toolset_id FK→toolset ON DELETE CASCADE, service_id FK→mcp_service, composite FK `(toolset_id,service_id)→toolset(id,service_id)`, name String(128) UNIQUE(toolset_id,name), title String(256) nullable, description LONG_TEXT nullable, input_schema/raw_definition JSON_DOC NOT NULL, output_schema/annotations/execution/icons/meta/http_binding JSON_DOC nullable, definition_digest String(64), source ENUM_CODE manual/synced+CHECK, enabled Boolean server_default true, full §3.2 audit). Alembic revision `m1_model_004_toolset` extends from `m1_model_003_config_revision`; `make test-db-contract TEST=models_toolset` → 12 passed on BOTH real dialects via fresh-DB `upgrade head`. Both the toolset_id FK and the composite FK carry ON DELETE CASCADE (implementer-verified: a NO ACTION composite FK blocks the cascade on PostgreSQL). `mcp_service.active_toolset_id` composite FK is now structurally enabled by toolset UNIQUE(id,service_id) but enforcement stays deferred (§5.2 L165). Previous: `M1-MODEL-003` 配置 Revision 模型; `M1-MODEL-002` 服务模型; `M1-MODEL-001` user/team model contract; `M1-DB-003` Alembic migration system; `M1-DB-002` cross-dialect base types; `M1-DB-001` async session factory).
